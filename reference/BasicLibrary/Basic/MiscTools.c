...
void BL_SwapU8S(U8* A,U8* B,S32 Length)
{
	...
	if(Length>MaxLen)
		return;
	BL_MemCopy(T,A, Length);
	BL_MemCopy(A,B, Length);
	BL_MemCopy(B,A, Length);
}

char FormatBuffer[FORMATBUFFMAX] = {0, };
...
U8* BL_FormatString(const char* format, ...)
{
	...
	vsprintf(FormatBuffer, format, v_list);
	va_end(v_list);

	return (U8*)FormatBuffer;
}

StandMessageSend DebugMessageSendFun = Null;

S32 BL_SendDebugString(const char* format, ...)
{
	va_list v_list;
	if(DebugMessageSendFun == Null)
		return 0;
	va_start(v_list, format);
	vsprintf(FormatBuffer, format, v_list);
	va_end(v_list);
	return DebugMessageSendFun(FormatBuffer);
}
...
StandMemAllocEx ExMemAllocFun = Null;

void* BL_MemAllocEx(S32 Length)
{
	if(ExMemAllocFun != Null)
		return ExMemAllocFun(Length,False);
	return BL_MemAlloc(Length);
}

void* BL_MemAllocExTemp(S32 Length)
{
	if(ExMemAllocFun != Null)
		return ExMemAllocFun(Length,True);
	return BL_MemAlloc(Length);
}
...
U32 BL_CRC32Encode(U32 CRC, U8* Buffer, S32 BufferLen)
{
	CRC = CRC ^ ~0U;
	while (BufferLen--)
		CRC = crc32_tab[(CRC ^ *Buffer++) & 0xFF] ^ (CRC >> 8);
...
/*
	DataLength为待处理数据块数(每块U32=32Bit),>1加密,<-1解密.
	Key=4*U32=128Bit
*/
void BL_XXTEA(U32* Buffer, S32 BufferLen, U32* Key)
{
	U32 z = Buffer[BufferLen-1], y = Buffer[0], sum = 0, e, DELTA = 0x9e3779b9;
	S32 p, q ;
	if (BufferLen > 1)
	{
		q = 6 + 52 / BufferLen;
		while (q-- > 0)
		{
			sum += DELTA;
			e = (sum >> 2) & 3;
			for (p = 0; p < BufferLen - 1; p++) y = Buffer[p+1], z = Buffer[p] += XXTEA_MX;
			y = Buffer[0];
			z = Buffer[BufferLen-1] += XXTEA_MX;
		}
	}
	else if (BufferLen < -1)
	{
		...
		{
			e = (sum >> 2) & 3;
			for (p = BufferLen - 1; p > 0; p--) z = Buffer[p-1], y = Buffer[p] -= XXTEA_MX;
			z = Buffer[BufferLen-1];
			y = Buffer[0] -= XXTEA_MX;
			sum -= DELTA;
		}
	}
...
F32 BL_ABSAdject(F32 Value,F32 Adject)
{
	...
	{
		return Value+BL_Sign(Value)*Adject;
	}
	else
	{
		if(BL_ABS(Value)<=BL_ABS(Adject))
		{
			return 0;
		}
		else
