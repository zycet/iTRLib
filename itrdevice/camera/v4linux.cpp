...
int v4linux::capture_get_output_ptr (void *c, unsigned char***ptr, int **ls)
{
    Ctx *ctx = (Ctx*)c;
...
int v4linux::capture_get_picture (void *id, v4linux::Picture *pic)
{
	// 获取, 转换
	Ctx *ctx = (Ctx*)id;
	v4l2_buffer buf;
...
int v4linux::capture_close (void *id)
{
	Ctx *ctx = (Ctx*)id;
	for (int i = 0; i < sizeof(ctx->bufs)/sizeof(Buffer); i++) {
